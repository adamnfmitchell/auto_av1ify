...
for output in chunk_*.y4m; do
  rav1e -y -b 600 -i 120 -s 8 --tile-rows 4 --tile-cols 4 $output --output $output.b600.s8.4x4.ivf &
  nrwait 8
done
wait
...
for f in ./*b600.s8.4x4.ivf; do echo "file '$f'" >> chunks.txt; done
## Combine them into one video
ffmpeg -f concat -safe 0 -i chunks.txt -metadata codec="AV1" -metadata bit_rate="600" -c copy "$1.noaudio.mkv" &&
## Mux with the video into one MKV
ffmpeg -i "$1.opus" -i "$1.noaudio.mkv" -c copy "$1.mkv" &&
mv "./$1.mkv" "../done/$1.av1.b600.s8.4x4.mkv" &&
cd .. &&
rm -rf temp_chunks/ &&
rm "$1"
wait